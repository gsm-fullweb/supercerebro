    ...
    try {
        ...
        try {
            ...
            if (conversation === null) {
                // 404 confirmado — conversa realmente deletada no Chatwoot
                logger_1.default.warn('Conversation not found in Chatwoot (404 — deleted)', { conversationId });
                conversationDeleted = true;
            }
        }
        catch (convError) {
            // Erro transitório (403, 5xx, timeout) — não marca como deletada, apenas loga
            logger_1.default.warn('Transient error fetching conversation from Chatwoot', { conversationId });
        }
        ...
        // Enriquece a conversa com channel_type da inbox (Chatwoot não retorna no payload da conversa)
        const rawInboxId = conversation?.inbox_id;
        let inboxChannelType = null;
        if (rawInboxId) {
            // Tenta banco primeiro (mais rápido), depois API como fallback
            inboxChannelType = await chatwootDatabase_1.default.getInboxChannelType(rawInboxId);
            if (!inboxChannelType) {
                const inboxData = await chatwoot_1.default.getInboxById(accountId, rawInboxId, authReq.jwt['access-token'] ? authReq.jwt : undefined, authReq.jwt['access-token'] ? undefined : authReq.apiToken);
                inboxChannelType = inboxData?.channel_type || null;
            }
        }
        // Se conversa deletada, cria objeto mock com dados do card
        const conversationData = conversation
            ? { ...conversation, inbox: { id: rawInboxId, channel_type: inboxChannelType || '' } }
            : (conversationDeleted && card ? {
                id: conversationId,
                status: 'deleted',
                priority: null,
                meta: {
                ...
                inbox: { id: 0, name: 'Desconhecida', channel_type: '' },
                labels: [],
...
router.post('/:id/scheduled-messages', upload.single('attachment'), async (req, res) => {
    const authReq = req;
    const conversationId = parseInt(req.params.id);
    const { message, scheduledAt, messageType, templateName, templateParams } = req.body;
    const isTemplate = messageType === 'template';
    // Template: requer nome do template; texto: requer mensagem ou anexo
    if (isTemplate && !templateName?.trim()) {
        return res.status(400).json({ error: 'Nome do template é obrigatório' });
    }
    if (!isTemplate && !message?.trim() && !req.file) {
    ...
    try {
        ...
        const scheduledMessage = await database_1.default.scheduledMessage.create({
            data: {
                ...
                templateName: isTemplate ? templateName?.trim() : undefined,
                templateParams: isTemplate && templateParams ? (typeof templateParams === 'string' ? templateParams : JSON.stringify(templateParams)) : undefined,
                ...authData
            }
        });
...
router.patch('/:id/scheduled-messages/:msgId', upload.single('attachment'), async (req, res) => {
    ...
    const { message, scheduledAt, removeAttachment, templateParams } = req.body;
    try {
        ...
        const updateData = {};
        // Se reagendando uma mensagem que falhou, reseta o status para pending
        ...
        if (templateParams !== undefined) {
            updateData.templateParams = typeof templateParams === 'string' ? templateParams : JSON.stringify(templateParams);
        }
