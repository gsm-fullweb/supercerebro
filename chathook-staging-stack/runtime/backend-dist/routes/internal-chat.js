...
router.get('/', async (req, res) => {
    ...
    try {
        ...
        // Calcula unreadCount para cada chat
        const chatsWithUnread = await Promise.all(chats.map(async (chat) => {
            const membership = chat.members.find(m => m.userId === userId);
            const lastReadAt = membership?.lastReadAt;
            const unreadCount = await database_1.default.internalChatMessage.count({
                where: {
                    chatId: chat.id,
                    userId: { not: userId },
...
router.post('/', async (req, res) => {
    ...
    try {
        // Garante que o criador também é membro (sem duplicar se já estiver na lista)
        ...
        logger_1.default.info('Internal chat created', { chatId: chat.id, userId, accountId, memberCount: allMemberIds.length });
...
router.post('/:id/messages', upload.array('files', 5), async (req, res) => {
    ...
    try {
        ...
        if (io) {
            ...
            io.to(`chat_${chatId}`).emit('new_message', { chatId, message: messagePayload });
            logger_1.default.debug('WebSocket event emitted', { event: 'new_message', chatId, messageId: message.id });
            // Notifica via sala pessoal todos os membros que não estão visualizando o chat
            const allMembers = await database_1.default.internalChatMember.findMany({ where: { chatId } });
            const ioRef = io;
            allMembers.forEach(member => {
                if (member.userId !== userId) {
                    ioRef.to(`user_${member.userId}`).emit('new_message_notification', { chatId });
                }
            });
        }
...
// POST /api/internal-chats/:id/read - Marca chat como lido para o usuário atual
router.post('/:id/read', async (req, res) => {
    ...
    if (isNaN(chatId))
        return res.status(400).json({ error: 'ID do chat inválido' });
    try {
        await database_1.default.internalChatMember.update({
        ...
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        logger_1.default.error('Failed to mark chat as read', { chatId, userId, error: errorMessage });
