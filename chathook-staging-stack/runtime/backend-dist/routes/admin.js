...
router.get('/allowed-providers', async (req, res) => {
    try {
        ...
        let allowedProviders = ['evolution', 'waha', 'uazapi', 'evolution-go']; // Padrão: todos permitidos
...
// PATCH /api/admin/allowed-providers - Atualiza provedores permitidos
// SuperAdmins podem passar accountId no body para atualizar outra empresa
router.patch('/allowed-providers', checkAdminRole, async (req, res) => {
    try {
        const authReq = req;
        const { providers, accountId: bodyAccountId } = req.body;
        // SuperAdmin pode especificar qual empresa atualizar via body.accountId
        let accountId = authReq.user.account_id;
        if (bodyAccountId && authReq.user.type === 'SuperAdmin') {
            const parsed = parseInt(bodyAccountId);
            if (!isNaN(parsed))
                accountId = parsed;
        }
        ...
        // Valida que apenas evolution, waha, uazapi e evolution-go são permitidos
        const validProviders = providers.filter((p) => ['evolution', 'waha', 'uazapi', 'evolution-go'].includes(p));
...
// ==========================================
// PWA CONFIG (SuperAdmin only — accountId = 0)
// ==========================================
// GET /api/admin/pwa-config
router.get('/pwa-config', async (req, res) => {
    try {
        const authReq = req;
        if (authReq.user?.type !== 'SuperAdmin') {
        ...
        res.status(500).json({ error: 'Erro ao buscar configuração PWA' });
    }
});
// PUT /api/admin/pwa-config
router.put('/pwa-config', async (req, res) => {
    try {
        const authReq = req;
        if (authReq.user?.type !== 'SuperAdmin') {
            return res.status(403).json({ error: 'Acesso restrito a SuperAdmin' });
        }
        const { pwaAppName, pwaLogoUrl, pwaIconUrl, pwaThemeColor } = req.body;
        await database_1.default.systemSettings.upsert({
            where: { accountId: 0 },
            update: { pwaAppName, pwaLogoUrl, pwaIconUrl, pwaThemeColor },
            create: { accountId: 0, pwaAppName, pwaLogoUrl, pwaIconUrl, pwaThemeColor },
        });
        res.json({ success: true });
    }
    catch (err) {
        res.status(500).json({ error: 'Erro ao salvar configuração PWA' });
    }
});
