...
// GET /api/chatbot-flows/sla-policies - Busca SLA policies da conta
router.get('/chatbot-flows/sla-policies', async (req, res) => {
    try {
        const authReq = req;
        const { account_id } = authReq.user;
        const policies = await chatwoot_1.default.getSLAPolicies(account_id, authReq.jwt, authReq.apiToken);
        res.json({ data: policies });
    }
    catch (error) {
        logger_1.default.error('Error fetching SLA policies:', error);
        res.status(500).json({ error: 'Erro ao buscar SLA policies' });
    }
});
...
router.put('/chatbot-flows/:id', async (req, res) => {
    try {
        ...
        // Garante webhook global ao ativar ou editar trigger do flow
        if (updates.isActive || updates.trigger) {
            await (0, globalWebhook_1.ensureGlobalWebhook)(account_id, authReq.apiToken, authReq.jwt);
        }
        const flowWithParsedData = {
            ...updatedFlow,
