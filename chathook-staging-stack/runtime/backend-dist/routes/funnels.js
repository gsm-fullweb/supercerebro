...
// Cria ou retorna o funil de sistema (Status Tickets)
...
router.get('/', async (req, res) => {
    ...
    try {
        ...
        const funnels = userIsAdmin
            ? allFunnels
            : allFunnels.filter(f => {
                const passes = f.isPublic || f.allowedUsers.some(u => Number(u.userId) === Number(userId));
                if (!passes) {
                    logger_1.default.info('Funnel filtered out for non-admin', {
                        funnelId: f.id,
                        funnelName: f.name,
                        isPublic: f.isPublic,
                        isActive: f.isActive,
                        allowedUserIds: f.allowedUsers.map(u => u.userId),
        ...
        logger_1.default.info('GET /api/funnels result', {
            accountId,
            userId,
            userIdType: typeof userId,
...
router.put('/:id', async (req, res) => {
    ...
    try {
        ...
        if (allowedUserIds !== undefined) {
            ...
            (0, kanban_1.invalidateFunnelBoardCache)(accountId, funnelId);
            (0, kanban_1.invalidateCardsIndexCache)(accountId);
...
// POST /api/funnels/:id/duplicate - Duplica funil com todas as etapas (apenas admin)
router.post('/:id/duplicate', async (req, res) => {
    if (!isAdmin(req)) {
        return res.status(403).json({ error: 'Apenas administradores podem duplicar funis' });
    }
    const authReq = req;
    const accountId = authReq.user.account_id;
    const funnelId = parseInt(req.params.id);
    try {
        const source = await database_1.default.funnel.findFirst({
            where: { id: funnelId, accountId },
        ...
        const count = await database_1.default.funnel.count({ where: { accountId } });
        const copy = await database_1.default.funnel.create({
            data: {
        ...
        logger_1.default.info('Funnel duplicated', { sourceFunnelId: funnelId, newFunnelId: copy.id, accountId });
        res.status(201).json({ success: true, data: copy });
    }
    catch (error) {
        logger_1.default.error('Error duplicating funnel', { error });
...
router.put('/:funnelId/stages/:stageId', async (req, res) => {
    ...
    try {
        ...
        // Se newTicket foi ativado, garante que o webhook global existe no Chatwoot
        if (automations?.newTicket === true) {
            (0, globalWebhook_1.ensureGlobalWebhook)(accountId, authReq.apiToken, authReq.jwt).catch(err => {
                logger_1.default.warn('ensureGlobalWebhook falhou ao salvar newTicket (não crítico)', { error: err });
            });
        }
...
router.post('/standalone-cards', async (req, res) => {
    ...
    try {
        ...
        (0, kanban_1.invalidateFunnelBoardCache)(accountId, stage.funnel.id);
        res.json({ success: true, data: card });
    }
    catch (error) {
...
router.patch('/standalone-cards/:id', async (req, res) => {
    ...
    try {
        ...
        const oldFunnelId = card.stage.funnel.id;
        ...
        (0, kanban_1.invalidateFunnelBoardCache)(accountId, oldFunnelId);
        if (stageId) {
            const newStage = await database_1.default.stage.findFirst({
                where: { id: parseInt(stageId), funnel: { accountId } },
                include: { funnel: true }
            });
            if (newStage && newStage.funnel.id !== oldFunnelId) {
                (0, kanban_1.invalidateFunnelBoardCache)(accountId, newStage.funnel.id);
            }
        }
...
router.delete('/standalone-cards/:id', async (req, res) => {
    ...
    try {
        ...
        if (!card)
            return res.status(404).json({ error: 'Card não encontrado' });
        const funnelId = card.stage.funnel.id;
