...
exports.AVAILABLE_PERMISSIONS = [
    ...
    // Permissões de Empresa (Super Admin apenas)
    { value: 'account-permissions:read', label: 'Permissões de Empresa: Leitura', description: 'Listar e consultar permissões de módulos por empresa (requer Super Admin)' },
    { value: 'account-permissions:write', label: 'Permissões de Empresa: Escrita', description: 'Habilitar/desabilitar módulos por empresa (requer Super Admin)' },
];
...
router.post('/', async (req, res) => {
    ...
    // Escopos restritos a Super Admin — impede que usuários comuns criem tokens com poderes elevados
    const restrictedScopes = ['account-permissions:read', 'account-permissions:write'];
    const needsSuperAdmin = permissions.some((p) => restrictedScopes.includes(p));
    if (needsSuperAdmin) {
        try {
            const profile = await chatwoot_1.default.getUserProfile(authReq.jwt, authReq.apiToken);
            if (profile?.type !== 'SuperAdmin') {
                return res.status(403).json({ error: 'Apenas Super Admins podem criar tokens com escopos de permissões de empresa (account-permissions)' });
            }
        }
        catch {
            return res.status(403).json({ error: 'Não foi possível verificar se o usuário é Super Admin' });
        }
    }
    try {
