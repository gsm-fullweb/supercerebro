...
router.get('/conversations/:conversationId/items', async (req, res) => {
    ...
    try {
        ...
        // Buscar o card da conversa — fallback para cards avulsos (conversationId = null)
        let card = await database_1.default.card.findFirst({
            where: { conversationId: convIdInt, accountId }
        });
        if (!card) {
            card = await database_1.default.card.findFirst({
                where: { id: convIdInt, accountId, conversationId: null }
            });
        }
        if (!card) {
...
router.put('/conversations/:conversationId/items/:id', async (req, res) => {
    ...
    try {
        ...
        const convIdInt = parseInt(conversationId);
        // Buscar card (fallback avulso) para usar o cardId na verificação
        let card = await database_1.default.card.findFirst({ where: { conversationId: convIdInt, accountId } });
        if (!card) {
            card = await database_1.default.card.findFirst({ where: { id: convIdInt, accountId, conversationId: null } });
        }
        // Verificar se o item existe e pertence à conta
        const existingItem = await database_1.default.cardItem.findFirst({
            where: {
                id: parseInt(id),
                ...(card ? { cardId: card.id } : { conversationId: convIdInt }),
                accountId
            }
        });
        if (!existingItem) {
...
router.get('/conversations/:conversationId/items/total', async (req, res) => {
    ...
    try {
        ...
        const accountId = authReq.user.account_id;
        const convIdInt = parseInt(conversationId);
        // Buscar o card da conversa — fallback para cards avulsos
        let card = await database_1.default.card.findFirst({
            where: { conversationId: convIdInt, accountId }
        });
        if (!card) {
