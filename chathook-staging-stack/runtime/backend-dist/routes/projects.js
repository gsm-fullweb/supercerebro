...
router.get('/:id/details', async (req, res) => {
    try {
        ...
        // Enriquece conversas com contactName buscado da tabela Card
        const convIds = project.conversations.map(c => c.conversationId);
        const cards = convIds.length > 0
            ? await prisma.card.findMany({
                where: { conversationId: { in: convIds }, accountId },
                select: { conversationId: true, customName: true }
            })
            : [];
        const cardNameMap = new Map(cards.map(c => [c.conversationId, c.customName]));
        res.json({
            success: true,
            data: {
                ...project,
                conversations: project.conversations.map(c => ({
                    ...c,
                    contactName: cardNameMap.get(c.conversationId) || null
                }))
            }
        });
    }
