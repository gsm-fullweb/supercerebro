...
router.get('/cards/:conversationId/custom-fields', async (req, res) => {
    try {
        ...
        // Busca por conversationId (card normal) ou por id direto (card avulso sem ticket Chatwoot)
        let card = await database_1.default.card.findFirst({
            where: { conversationId, accountId },
            include: { stage: true }
        });
        if (!card) {
            // Fallback: card avulso — frontend envia o card.id como "conversationId"
            card = await database_1.default.card.findFirst({
                where: { id: conversationId, accountId, conversationId: null },
                include: { stage: true }
            });
        }
...
router.put('/cards/:conversationId/custom-fields/:fieldId', async (req, res) => {
    try {
        ...
        // Busca por conversationId (card normal) ou por id direto (card avulso)
        ...
        const fieldValue = await database_1.default.customFieldValue.upsert({
            ...
            create: { cardId: card.id, fieldId, conversationId: card.conversationId ?? 0, accountId, value },
            update: { value }
        });
