...
async function validateAuth(req, res, next) {
    // Idempotência: se já autenticado nesta requisição (por chamada anterior do mesmo middleware), pula
    if (req.user) {
        return next();
    }
    ...
    // Verifica KanbanCW Bearer token (API Token interno)
    const authHeader = req.headers.authorization;
    if (authHeader && authHeader.startsWith('Bearer ')) {
        const bearerToken = authHeader.substring(7);
        try {
            const apiToken = await database_1.default.apiToken.findUnique({ where: { token: bearerToken } });
            if (apiToken && apiToken.isActive && (!apiToken.expiresAt || apiToken.expiresAt > new Date())) {
                const authReq = req;
                // Constrói user mínimo a partir dos dados do token
                authReq.user = {
                    id: apiToken.userId,
                    account_id: apiToken.accountId,
                    name: 'API Token',
                    email: '',
                    type: 'agent',
                    role: 'agent',
                    accounts: [{ account_id: apiToken.accountId, role: 'agent' }],
                    access_token: '',
                };
                authReq.apiToken = undefined;
                authReq.jwt = { 'access-token': '', 'token-type': 'Bearer', client: '', expiry: '', uid: '' };
                authReq.accountId = apiToken.accountId;
                authReq.userId = apiToken.userId;
                // Atualiza lastUsedAt do token
                database_1.default.apiToken.update({ where: { id: apiToken.id }, data: { lastUsedAt: new Date() } }).catch(() => { });
                logger_1.default.info('Authenticated with KanbanCW Bearer token', {
                    tokenId: apiToken.id,
            ...
            logger_1.default.warn('KanbanCW Bearer token lookup failed', { error: err instanceof Error ? err.message : err, path: req.path });
        }
        // Bearer inválido/expirado: continua para tentar outros métodos
    }
    // Verifica API access token primeiro (Chatwoot)
