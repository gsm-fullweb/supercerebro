...
exports.deleteWahaMessage = deleteWahaMessage;
exports.editWahaMessage = editWahaMessage;
exports.findWahaMessageId = findWahaMessageId;
 ...
 */
async function deleteWahaSession(sessionName, accountId) {
    try {
        const client = accountId ? await getWahaClient(accountId) : wahaClient;
        await client.delete(`/api/sessions/${sessionName}`);
 ...
 */
async function logoutWahaSession(sessionName, accountId) {
    try {
        const client = accountId ? await getWahaClient(accountId) : wahaClient;
        await client.post(`/api/sessions/${sessionName}/logout`);
 ...
 */
async function restartWahaSession(sessionName, accountId) {
    try {
        const client = accountId ? await getWahaClient(accountId) : wahaClient;
        // Tenta /restart primeiro (funciona para FAILED/DISCONNECTED/STARTING/STOPPED)
        // /start só funciona para STOPPED — não resolve sessões em FAILED
        try {
            await client.post(`/api/sessions/${sessionName}/restart`);
            logger_1.default.info('Waha session restarted via /restart', { sessionName });
            return true;
        }
        catch (restartError) {
            // Se /restart falhar (WAHA mais antigo sem esse endpoint), tenta /stop + /start
            logger_1.default.debug('Waha /restart failed, trying stop+start', { sessionName, error: restartError.message });
            try {
                await client.post(`/api/sessions/${sessionName}/stop`);
                await new Promise(r => setTimeout(r, 1000));
            }
            catch { /* ignora erro no stop */ }
            await client.post(`/api/sessions/${sessionName}/start`);
            logger_1.default.info('Waha session restarted via stop+start', { sessionName });
            return true;
        }
    }
    catch (error) {
 ...
 */
async function deleteWahaMessage(accountId, sessionName, chatId, messageId) {
    const client = await getWahaClient(accountId);
    await client.delete(`/api/${sessionName}/chats/${chatId}/messages/${encodeURIComponent(messageId)}`);
    logger_1.default.info('WAHA: mensagem apagada', { sessionName, messageId });
}
async function editWahaMessage(accountId, sessionName, chatId, messageId, newText) {
    const client = await getWahaClient(accountId);
    await client.put(`/api/${sessionName}/chats/${chatId}/messages/${encodeURIComponent(messageId)}`, {
        text: newText,
    });
    logger_1.default.info('WAHA: mensagem editada', { sessionName, messageId });
}
// Gera variações de chatId para BR (9 dígitos ↔ 8 dígitos)
function brChatIdVariants(chatId) {
    const ids = [chatId];
    const phone = chatId.replace('@c.us', '');
    // 13 dígitos (55 + DDD 2 + 9 dígitos): tentar sem o 9 extra → 12 dígitos
    if (phone.startsWith('55') && phone.length === 13 && phone[4] === '9') {
        ids.push(`${phone.slice(0, 4)}${phone.slice(5)}@c.us`);
    }
    // 12 dígitos (55 + DDD 2 + 8 dígitos): tentar com o 9 extra → 13 dígitos
    if (phone.startsWith('55') && phone.length === 12) {
        ids.push(`${phone.slice(0, 4)}9${phone.slice(4)}@c.us`);
    }
    return ids;
}
// Busca o ID WhatsApp de uma mensagem enviada quando source_id está vazio no Chatwoot
async function findWahaMessageId(accountId, sessionName, chatId, content, createdAtSeconds) {
    const client = await getWahaClient(accountId);
    const contentClean = content.trim().toLowerCase();
    const candidates = brChatIdVariants(chatId);
    for (const cid of candidates) {
        try {
            const res = await client.get(`/api/${sessionName}/chats/${cid}/messages`, {
                params: {
                    limit: 100,
                    downloadMedia: false,
                    'filter.fromMe': true,
                    'filter.timestamp.gte': createdAtSeconds - 600,
                    'filter.timestamp.lte': createdAtSeconds + 600,
                },
            });
            const messages = Array.isArray(res.data) ? res.data : (res.data?.messages || []);
            logger_1.default.info('WAHA: findMessages resposta', {
                sessionName, chatId: cid, count: messages.length,
                firstMsg: messages[0] ? { id: messages[0].id, timestamp: messages[0].timestamp } : null,
            });
            const match = messages.find((m) => {
                if (!m.fromMe)
                    return false;
                const ts = m.timestamp || 0;
                if (Math.abs(ts - createdAtSeconds) > 600)
                    return false;
                const msgContent = (m.body || m.text || m.caption || m.content || '').trim().toLowerCase();
                return msgContent === contentClean;
            });
            if (match?.id) {
                logger_1.default.info('WAHA: source_id encontrado', { sessionName, chatId: cid, id: match.id });
                return match.id;
            }
        }
        catch (err) {
            logger_1.default.warn('WAHA: findWahaMessageId falhou para chatId', { sessionName, chatId: cid, error: err.message });
        }
    }
    logger_1.default.warn('WAHA: mensagem não encontrada no histórico', { sessionName, chatId, candidates, content: content.substring(0, 50), createdAtSeconds });
    return null;
}
