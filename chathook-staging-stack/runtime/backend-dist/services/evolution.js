...
class EvolutionAPI {
    ...
    async getInstanceInfo(accountId, instanceName) {
        try {
            ...
            const raw = response.data[0] || response.data;
            const status = raw.connectionStatus || raw.instance?.connectionStatus || raw.instance?.status || 'unknown';
            return {
                instanceName: raw.instance?.instanceName || raw.instanceName || instanceName,
                name: raw.instance?.instanceName || raw.instanceName || instanceName,
                state: status,
                connectionStatus: status,
                owner: raw.ownerJid?.replace(/@s\.whatsapp\.net$/, '') || raw.owner || null,
                profileName: raw.profileName || raw.instance?.profileName || null,
                profilePictureUrl: raw.profilePictureUrl || raw.instance?.profilePictureUrl || null,
    ...
    async deleteMessage(accountId, instanceName, messageId, remoteJid) {
        const { client } = await this.getClient(accountId);
        ...
        logger_1.default.info('Evolution: tentando apagar mensagem', { instanceName, messageId, jid });
        try {
            const res = await client.delete(`/chat/deleteMessage/${instanceName}`, { data: payload });
            logger_1.default.info('Evolution: mensagem apagada', { instanceName, messageId, status: res.status });
        }
        catch (err) {
            logger_1.default.error('Evolution: deleteMessage falhou', {
                instanceName, messageId, jid,
    ...
    async editMessage(accountId, instanceName, messageId, remoteJid, newText) {
        const { client } = await this.getClient(accountId);
        const jid = remoteJid.includes('@') ? remoteJid : `${remoteJid}@s.whatsapp.net`;
        // Chatwoot armazena source_id com prefixo "WAID:" para Evolution — remover
        const cleanId = messageId.startsWith('WAID:') ? messageId.slice(5) : messageId;
        // number deve ser só dígitos (sem @domain)
        const phoneOnly = remoteJid.replace(/[^0-9]/g, '');
        const payload = {
            number: phoneOnly,
            text: newText,
            key: {
        ...
        logger_1.default.info('Evolution: tentando editar mensagem', { instanceName, messageId, jid, payload });
        try {
            const res = await client.post(`/chat/updateMessage/${instanceName}`, payload);
            logger_1.default.info('Evolution: mensagem editada', { instanceName, messageId, status: res.status, data: res.data });
        }
        catch (err) {
            logger_1.default.error('Evolution: editMessage falhou', {
                instanceName, messageId, jid,
                status: err.response?.status,
                data: err.response?.data,
                message: err.message,
    ...
    // Usa timestamp + conteúdo como filtro
    async findMessageId(accountId, instanceName, remoteJid, content, createdAtSeconds) {
        try {
            const { client } = await this.getClient(accountId);
            const jid = remoteJid.includes('@') ? remoteJid : `${remoteJid}@s.whatsapp.net`;
            // Busca mensagens de saída num intervalo de ±10 min ao redor do timestamp
            const res = await client.post(`/chat/findMessages/${instanceName}`, {
                where: {
                    key: { remoteJid: jid, fromMe: true },
                    messageTimestamp: {
                        gte: createdAtSeconds - 600,
                        lte: createdAtSeconds + 600,
                    },
                },
                limit: 50,
            });
            const messages = res.data?.messages?.records || res.data || [];
            // Tenta casar pelo conteúdo da mensagem
            ...
            if (match?.key?.id) {
                logger_1.default.info('Evolution: source_id encontrado via findMessages', { instanceName, id: match.key.id });
                return match.key.id;
            }
            return null;
        }
        catch (err) {
            // Erros de conexão (DNS, timeout, ECONNREFUSED) indicam URL da Evolution mal configurada
            const isConnectionError = ['ENOTFOUND', 'ECONNREFUSED', 'ETIMEDOUT', 'ECONNRESET'].includes(err.code);
            if (isConnectionError) {
                logger_1.default.error('Evolution: API inacessível — URL provavelmente mal configurada', { instanceName, url: err.config?.baseURL, code: err.code });
                throw new Error(`Evolution API inacessível (${err.code}) — verifique a URL configurada nas Conexões`);
            }
            logger_1.default.warn('Evolution: findMessageId falhou', { instanceName, error: err.message });
            return null;
        }
    }
}
