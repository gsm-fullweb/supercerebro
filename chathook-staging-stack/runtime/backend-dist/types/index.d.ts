...
export type FlowNodeType = 'start' | 'sendText' | 'sendImage' | 'sendVideo' | 'sendAudio' | 'sendFile' | 'condition' | 'switch' | 'delay' | 'sequenceDelay' | 'checkResponse' | 'changeStatus' | 'labels' | 'assign' | 'applySLA' | 'aiAgent' | 'knowledgeBase' | 'httpRequest' | 'waitForResponse' | 'input' | 'end' | 'sequenceEnd' | 'sendWATemplate' | 'scheduleAppointment' | 'cancelAppointment' | 'aiSchedulingAgent' | 'checkAvailability' | 'moveToStage' | 'sendWAInteractive' | 'generateImage';
...
export interface AIAgentNodeData {
    provider: 'openai' | 'groq' | 'openrouter' | 'openai_oauth';
    model: string;
    prompt: string;
    temperature?: number;
    maxTokens?: number;
